...
**Never use the native `fetch` API in this frontend — always use axios.** Client components call the backend through the shared `frontend/lib/axios.ts` instance (`api`, `withCredentials: true`, `baseURL: NEXT_PUBLIC_API_URL`). Server components (which need to forward the `token` cookie manually, since there's no browser cookie jar) use `frontend/lib/serverApi.ts`'s `createServerApi(token)`, an axios instance scoped to `API_URL` with the `Cookie` header already set. This is an explicit project convention, not a default — apply it to any new data-fetching code.
...
Dashboards are role-specific components (`AdminDashboard.tsx`, `MemberDashboard.tsx`, `TrainerDashboard.tsx`). `app/(main)/dashboard/page.tsx` is a server component that reads the `token` cookie directly via `next/headers`, calls the backend's `/auth/me` itself (via `createServerApi`, not `frontend/lib/axios.ts`), and picks which dashboard to render based on `user.role` — this bypasses `AuthContext`/`useAuth()` entirely, so the two auth mechanisms (server-side cookie-based calls here vs. client-side `AuthContext`) coexist and should be kept in sync when changing auth logic.

**Unread-notification badge is shared via context, not per-component state:** `frontend/context/NotificationContext.tsx` (wrapping `(main)/layout.tsx` inside `AuthProvider`) owns `unreadCount` and `refreshUnreadCount()`, and centralizes the Pusher subscriptions to the `announcements` and `trainer-{id}` channels. `Navbar.tsx` just reads `unreadCount`; any code that changes read-state server-side (mark as read, mark all as read) must call `refreshUnreadCount()` afterward or the navbar badge will drift from the DB. Don't reintroduce a second local `unreadCount` state / duplicate Pusher subscription in a component — the previous version did this and had two independent, unsynced copies of the same count.
